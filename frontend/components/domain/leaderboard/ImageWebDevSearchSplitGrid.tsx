...
interface SplitGridProps {
  imageWebDevData: LeaderboardRow[];
  searchData: LeaderboardRow[];
  loading: boolean;
  onViewCategoryClick: (category: string) => void;
}

export const ImageWebDevSearchSplitGrid: React.FC<SplitGridProps> = ({
  imageWebDevData,
  searchData,
  loading,
  onViewCategoryClick
  ...
  const getBrandLogo = (creator: string) => {
    ...
    if (lowerCreator === 'kimi' || lowerCreator === 'moonshot') return <span className="text-sm font-extrabold font-sans text-slate-850 select-none italic">K</span>;
        ...
        <div>
          ...
          <div className="p-4 space-y-3">
            {dataSlice.map((model, idx) => {
              ...
              const confIntervalString = model.confidenceInterval || '±5';
              const rawCIValue = parseFloat(confIntervalString.replace(/[^\d.]/g, '')) || 5;
