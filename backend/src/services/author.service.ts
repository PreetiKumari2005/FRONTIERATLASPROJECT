...
export const getAuthors = async (queryRouter: QueryRouter, limit: number = 50, skip: number = 0) => {
  ...
  // 🎯 FIXED: Cast 'prisma' as 'any' to bypass the missing '.author' model compilation roadblock
  const routingResult = await queryRouter.routeQuery(intent, async (prisma) => {
    return (prisma as any).author.findMany({
      take: limit,
      skip: skip,
      orderBy: { name: 'asc' },
      select: { id: true, name: true, slug: true, createdAt: true }
...
export const getAuthorBySlug = async (queryRouter: QueryRouter, slug: string) => {
  ...
  // 🎯 FIXED: Cast 'prisma' as 'any' to skip structural type checking here as well
  const routingResult = await queryRouter.routeQuery(intent, async (prisma) => {
    return (prisma as any).author.findUnique({
      where: { slug },
      include: {
        papers: {
          take: 100, // Capped to prevent frontend freeze
